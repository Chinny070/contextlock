...
export default function QuestDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) {
    return <div className="text-center py-20 text-slate-400">Loading quest...</div>;
  }

  if (!task) {
    return <div className="text-center py-20 text-red-400">Quest not found</div>;
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <Link href="/kwest/browse" className="inline-flex items-center gap-1 text-sm text-slate-400 hover:text-white">
        ...
        <Card className="p-5">
          ...
          <div className="flex items-center gap-3">
            <span className={`inline-flex items-center font-mono text-xs px-2 py-0.5 rounded-full border ${
              ...
              {SUBMISSION_STATUSES[mySubmission.status]}
            </span>
            {mySubmission.status === 1 && !mySubmission.claimed && (
              <Link href="/kwest/claim">
                <Button size="sm">Claim Reward</Button>
              </Link>
            )}
            {mySubmission.claimed && (
              <span className="text-xs text-blue-400">Reward claimed</span>
            )}
          </div>
        </Card>
      )}
      ...
      {canSubmit && (
        <Card className="p-6">
          ...
          <p className="text-sm text-slate-400 mb-4">
            Submit {PROOF_TYPES[task.proofType].toLowerCase()} proof to complete this quest.
          </p>
          {task.proofType === 0 ? (
            <Textarea
              value={proofData}
            ...
            <Input
              value={proofData}
              onChange={(e) => setProofData(e.target.value)}
              placeholder={task.proofType === 2 ? "QmYour...IPFSHash" : "https://..."}
            />
          )}
              ...
              <div key={sub.id} className="bg-slate-900/50 rounded-lg p-4 border border-slate-700/30">
                <div className="flex items-start justify-between mb-2">
                  <p className="text-xs text-slate-500">
                    {shortenAddress(sub.submitter)} &middot; {new Date(Number(sub.submittedAt) * 1000).toLocaleDateString()}
                  </p>
                  <span className={`inline-flex items-center font-mono text-xs px-2 py-0.5 rounded-full border ${
                    sub.status === 1
                      ? "bg-emerald-500/20 text-emerald-400 border-emerald-500/30"
