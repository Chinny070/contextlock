...
import {
  getKwestCoreWrite, ensureUsdcApproval, parseUsdc,
  PLATFORM_FEE_BPS, USDC_DECIMALS,
...
export default function CreateTaskPage() {
  ...
  async function handleCreate() {
    ...
    if (!title || !description || !rewardPool || !totalSlots) {
      return toast.error("Fill in all required fields");
    }
    if (!calculations.minRewardMet) {
      return toast.error("Minimum reward is 1 USDC per participant");
    }
      ...
      <Card className="p-6 space-y-4">
        ...
        <div className="bg-slate-900/50 rounded-lg p-4 border border-slate-700/50 space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span className={`font-medium ${calculations.minRewardMet ? "text-white" : "text-red-400"}`}>
              {calculations.perUser.toFixed(2)} USDC
            </span>
          </div>
          {!calculations.minRewardMet && calculations.pool > 0 && (
            <div className="flex items-center gap-1.5 text-xs text-red-400">
              <AlertCircle className="w-3.5 h-3.5" />
              Minimum 1 USDC per participant required
            </div>
          )}
          <div className="flex justify-between text-sm">
